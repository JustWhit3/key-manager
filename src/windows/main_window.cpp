...
namespace kmanager::window {
    ...
    void MainWindow::initStateMachine() {
        // Create states
        this->login_state = QSharedPointer<state::LoginState>(new state::LoginState(this));
        this->menu_state = QSharedPointer<state::MenuState>(new state::MenuState(this));
        this->p_manager_state =
            QSharedPointer<state::PasswordManagerState>(new state::PasswordManagerState(this->menu_state.get()));
        this->set_password_state = QSharedPointer<state::SetPasswordState>(new state::SetPasswordState(this));
        this->p_generator_state =
            QSharedPointer<state::PasswordGeneratorState>(new state::PasswordGeneratorState(this->menu_state.get()));
    ...
    void MainWindow::setAnimation() {
        ...
        QObject::connect(this->menu_state.get(), &QState::entered, this->menu_state->p_manager_button.get(), [this]() {
            ...
            this->a_MenuState_PasswordManagerState->setEndValue(
                QRect(this->menu_state->p_manager_button->geometry().x(),
                      this->menu_state->p_manager_button->geometry().y(), 220, 70));
        ...
        QObject::connect(this->menu_state.get(), &QState::entered, this->menu_state->p_generator_button.get(),
                         [this]() {
                             ...
                             this->a_MenuState_PasswordGeneratorState->setEndValue(
                                 QRect(this->menu_state->p_generator_button->geometry().x(),
                                       this->menu_state->p_generator_button->geometry().y(), 220, 70));
                             this->a_MenuState_PasswordGeneratorState->setDuration(300);
                             this->a_MenuState_PasswordGeneratorState->start();
                         });

        // Menu state -> exit
        QObject::connect(this->menu_state.get(), &QState::entered, this->menu_state->exit_button.get(), [this]() {
            this->a_MenuState_Exit = QSharedPointer<QPropertyAnimation>(
                new QPropertyAnimation(this->menu_state->exit_button.get(), "geometry"));
            this->a_MenuState_Exit->setStartValue(QRect(0, this->geometry().width() * 0.6f, 100, 100));
            this->a_MenuState_Exit->setEasingCurve(QEasingCurve::Linear);
            this->a_MenuState_Exit->setEndValue(QRect(this->menu_state->exit_button->geometry().x(),
                                                      this->menu_state->exit_button->geometry().y(), 220, 70));
    ...
    void MainWindow::buttonsPressedActions() {
        // Menu state -> Password manager state
        QObject::connect(this->menu_state->p_manager_button.get(), SIGNAL(clicked()), this,
                         SLOT(MenuState_PasswordManagerState()));

        // Password manager state -> Menu state
        QObject::connect(this->p_manager_state->menu_button.get(), SIGNAL(clicked()), this,
                         SLOT(PasswordManagerState_MenuState()));

        // Restart time loop when entering password manager state
        QObject::connect(this->menu_state->p_manager_button.get(), SIGNAL(clicked()), this->p_manager_state.get(),
                         SLOT(startTimeLoop()));

        // Login state -> Menu state
        QObject::connect(this->login_state.get(), SIGNAL(login_successful()), this, SLOT(LoginState_MenuState()));

        // Set password state -> Menu state
        QObject::connect(this->set_password_state.get(), SIGNAL(save_password_successful()), this,
                         SLOT(SetPasswordState_MenuState()));

        // Menu state -> Login state
        QObject::connect(this->menu_state->change_password_button.get(), SIGNAL(clicked()), this,
                         SLOT(MenuState_PasswordManagerState()));

        // Menu state -> Password generator state
        QObject::connect(this->menu_state->p_generator_button.get(), SIGNAL(clicked()), this,
                         SLOT(MenuState_PasswordManagerState()));

        // Password generator state -> Menu state
        QObject::connect(this->p_generator_state->menu_button.get(), SIGNAL(clicked()), this,
    ...
    void MainWindow::PasswordGeneratorState_MenuState() {
        ...
        this->menu_state->assignProperty(this->p_generator_state->ambiguous_characters_checkbox.get(), "visible",
                                         false);
