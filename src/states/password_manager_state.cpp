...
namespace kmanager::state {
    ...
    PasswordManagerState::PasswordManagerState(MenuState* host, QState* parent)
        : BaseState(parent), host(host), menu_button_pressed(false), repaint_passwords(false) {
        // Get passwords path
        ...
        std::filesystem::create_directories(this->password_dir.str());

        // Compute old passwords number in the password dir
        this->old_passwords_number = std::distance(std::filesystem::directory_iterator(this->password_dir.str()),
    ...
    void PasswordManagerState::setScrollArea() {
        ...
        this->scroll_area->setGeometry(0.f, 150.f, this->host->host->geometry().width(),
    ...
    void PasswordManagerState::addWidgets() {
        ...
        this->password_platform->move(
            0, this->host->host->mapToGlobal(this->host->host->geometry().center()).y() * 0.249f);
        ...
        this->password_username->move(this->password_platform->geometry().x() + this->label_width,
        ...
        this->password_key->move(this->password_username->geometry().x() + this->label_width,
        ...
        this->password_actions->move(this->password_key->geometry().x() + this->label_width,
        ...
        this->find_button->move(this->password_actions->geometry().x() + 205.f,
                                this->password_actions->geometry().y() - label_height);

        QObject::connect(this->find_button.get(), SIGNAL(clicked()), this, SLOT(findAction()));
        ...
        this->find_input->move(this->password_actions->geometry().x(),
                               this->password_actions->geometry().y() - label_height);
        this->find_input->setPlaceholderText("Search...");
        this->find_input->setContextMenuPolicy(Qt::NoContextMenu);

        QObject::connect(this->find_input.get(), SIGNAL(returnPressed()), this->find_button.get(),
                         SLOT(animateClick()));
        ...
        QObject::connect(this->menu_button.get(), SIGNAL(clicked()), this, SLOT(stopTimeLoop()));
        ...
        this->add_password_button->move(this->menu_button->geometry().x() + 101.f, this->menu_button->geometry().y());
        ...
        QObject::connect(this->add_password_button.get(), SIGNAL(clicked()), this, SLOT(addPassword()));
        ...
        this->n_of_passwords->setStyleSheet("font-size: 23px;"
                                            // "background-color: #ff9c2b;"
        );
        this->n_of_passwords->move(this->add_password_button->geometry().x() + 101.f,
    ...
    void PasswordManagerState::addPassword() {
        this->add_password_window = QSharedPointer<window::AddPasswordWindow>(new window::AddPasswordWindow(330));
    ...
    void PasswordManagerState::displayPasswords() {
        ...
        for (const auto& file: std::filesystem::directory_iterator(this->password_dir.str())) {
            ...
            utility::Crypto crypto_password(json_obj.value(QString("Password")).toString().toStdString(),
                                            this->file_key);
            utility::Crypto crypto_username(json_obj.value(QString("Username")).toString().toStdString(),
                                            this->file_key);
            ...
            QObject::connect(this->current_password_actions->delete_password.get(), SIGNAL(clicked()),
                             this->current_password_actions, SLOT(setDeleteTrue()));

            QObject::connect(this->current_password_actions->delete_password.get(), SIGNAL(clicked()), this,
                             SLOT(deleteMachinery()));

            QObject::connect(this->current_password_actions->save_password.get(), SIGNAL(clicked()),
                             this->current_password_actions, SLOT(setSaveMySettingsTrue()));

            QObject::connect(this->current_password_actions->save_password.get(), SIGNAL(clicked()), this,
                             SLOT(saveSettingsMachinery()));

            QObject::connect(this->current_password_actions->see_password_info.get(), SIGNAL(clicked()),
                             this->current_password_actions, SLOT(setInformationTrue()));

            QObject::connect(this->current_password_actions->see_password_info.get(), SIGNAL(clicked()), this,
                             SLOT(setInformationMachinery()));

            // Enter pressed
            QObject::connect(this->current_password_label, SIGNAL(returnPressed()),
                             this->current_password_actions->save_password.get(), SLOT(animateClick()));

            QObject::connect(this->current_username_label, SIGNAL(returnPressed()),
                             this->current_password_actions->save_password.get(), SLOT(animateClick()));

            QObject::connect(this->current_platform_label, SIGNAL(returnPressed()),
                             this->current_password_actions->save_password.get(), SLOT(animateClick()));
            ...
            std::sort(this->label_vec.begin(), this->label_vec.end(), [](const auto& a, const auto& b) {
    ...
    void PasswordManagerState::updatePasswordsView() {
        if (!menu_button_pressed) {
            ...
            this->current_passwords_number = std::distance(
                std::filesystem::directory_iterator(this->password_dir.str()), std::filesystem::directory_iterator{});
     ...
     *
     */
    void PasswordManagerState::stopTimeLoop() { this->menu_button_pressed = true; }
    ...
    void PasswordManagerState::deleteMachinery() {
        ...
        std::for_each(this->label_vec.cbegin(), this->label_vec.cend(), [this](const auto& el) {
            if (el.actions->deleteMe == true) {
                std::ostringstream password_to_remove;
                password_to_remove << this->password_dir.str() << el.username->text().toStdString() << ".json";
    ...
    void PasswordManagerState::setInformationMachinery() {
        ...
        std::for_each(this->label_vec.cbegin(), this->label_vec.cend(), [this](const auto& el) {
            if (el.actions->seeMyInfo == true) {
                ...
                this->add_password_window =
                    QSharedPointer<window::AddPasswordWindow>(new window::AddPasswordWindow(480, el.retrieveInfo()));
                this->add_password_window->show();
            }
        });
    }
    ...
    void PasswordManagerState::saveSettingsMachinery() {
        ...
        std::for_each(this->label_vec.cbegin(), this->label_vec.cend(), [this](const auto& el) {
            if (el.actions->saveMySettings == true) {
                ...
                main_container.insert("Password",
                                      QJsonValue::fromVariant(QString::fromStdString(crypto_password.encrypt())));
                main_container.insert("Username",
                                      QJsonValue::fromVariant(QString::fromStdString(crypto_username.encrypt())));
                ...
                main_container.insert("Strength",
                                      QJsonValue::fromVariant(this->evaluatePasswordStrength(el.password_str->text())));
                ...
                if (el.username->old_label != el.username->text()) {
                    ...
                    password_to_remove << this->password_dir.str() << el.username->old_label.toStdString() << ".json";
                    std::filesystem::remove(password_to_remove.str());
                }
            }
        });
