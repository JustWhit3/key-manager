...
namespace kmanager::state {
     ...
     */
    MenuState::MenuState(QWidget *host, QState *parent) : BaseState(parent), host(host) {
        // Create widgets
        this->addWidgets();
    ...
    void MenuState::addWidgets() {
        ...
        this->p_manager_button->move(
            ...
            (this->host->mapToGlobal(this->host->geometry().center()).y() -
             this->p_manager_button->mapToGlobal(this->p_manager_button->geometry().center()).y()) *
                0.9f);

        // Password generator button
        this->p_generator_button = QSharedPointer<QPushButton>(new QPushButton("Password generator", this->host));
        ...
        this->p_generator_button->move(this->p_manager_button->geometry().x(),
                                       this->p_manager_button->geometry().y() + 80.f);

        // Exit button
        this->exit_button = QSharedPointer<QPushButton>(new QPushButton("Exit", this->host));
        ...
        this->exit_button->move(this->p_generator_button->geometry().x(),
                                this->p_generator_button->geometry().y() + 80.f);

        QObject::connect(this->exit_button.get(), SIGNAL(clicked()), this->host, SLOT(close()));

        // Change password button
        this->change_password_button = QSharedPointer<QPushButton>(new QPushButton("", this->host));
        ...
        this->change_password_button->move(this->host->mapToGlobal(this->host->geometry().center()).x() * 1.84f,
        ...
        // Image logo label
        this->logo_img_label = QSharedPointer<QLabel>(new QLabel(this->host));
        this->logo_img_label->setVisible(false);
        this->logo_img_label->setPixmap(QPixmap(utility::getRealImgPath("img/logo_app.png")));
        this->logo_img_label->move(this->p_manager_button->geometry().x() * 0.86f,
                                   this->p_manager_button->geometry().y() * 0.2f);

        // Side rectangle decoration
        this->side_rectangle = QSharedPointer<QLabel>(new QLabel(this->host));
        this->side_rectangle->setVisible(false);
        this->side_rectangle->resize(this->host->geometry().width() * 0.35f, this->host->geometry().height());
