...
namespace kmanager::state{
     ...
     * @return A string representing the password strength as a percentage compared to 100%. The returned string has the format "XX.XX%", where XX.XX represents the percentage value.
     */
    QString BaseState::evaluatePasswordStrength( const QString& password ) {
        int score = 0;
        std::vector<QString> patterns = { "abcdefghijklmnopqrstuvwxyz",
        ...
        for ( const auto& pattern : patterns ) {
            if ( std::any_of( password.begin(), password.end(),
                            [ pattern ]( QChar c ) { return pattern.contains(c); } ) ) {
                score++;
            }
        }
