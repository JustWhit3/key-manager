...
namespace kmanager::utility {
     ...
     *
     */
    Crypto::Crypto(std::string_view message, std::string_view key) : message(message), key(key) {}
     ...
     *
     */
    Crypto::Crypto(std::string_view message) : message(message) {
        // Generate random key
    ...
    std::string Crypto::encrypt() {
        ...
        try {
            ...
            CryptoPP::AES::Encryption aesEncryption(reinterpret_cast<const CryptoPP::byte*>(this->key.data()),
            ...
            CryptoPP::StringSource(ciphertext, true,
                                   new CryptoPP::Base64Encoder(new CryptoPP::StringSink(encodedCiphertext), false));
    ...
    std::string Crypto::decrypt() {
        ...
        try {
            ...
            CryptoPP::StringSource(this->message, true,
                                   new CryptoPP::Base64Decoder(new CryptoPP::StringSink(decodedMessage)));
            ...
            CryptoPP::AES::Decryption aesDecryption(reinterpret_cast<const CryptoPP::byte*>(this->key.data()),
                                                    CryptoPP::AES::MAX_KEYLENGTH);
            CryptoPP::CBC_Mode_ExternalCipher::Decryption cbcDecryption(aesDecryption, iv);

            CryptoPP::StringSource(
                ciphertextWithoutIV.data(), true,
                new CryptoPP::StreamTransformationFilter(cbcDecryption, new CryptoPP::StringSink(decrypted)));
     ...
     *
     */
    std::string Crypto::getMessage() const { return this->message; }
     ...
     *
     */
    std::string Crypto::getKey() const { return this->key; }
     ...
     */
    void Crypto::setMessage(std::string_view message) { this->message = message; }
     ...
     */
    void Crypto::setKey(std::string_view key) { this->key = key; }
